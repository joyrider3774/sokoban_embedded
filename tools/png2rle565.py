...
The raw RGB565 full screen images (2 per skin, 32KB each) take more flash than the
smaller devices have, so they are stored compressed and drawn with pushImageRLE()
in GameFuncs.cpp.
...
defaults to ../assets/skins and ../source/sokoban_embedded/images next to this script
"""
import os
import sys
from PIL import Image

FULLSCREEN_IMAGES = ["background", "titlescreen"]
...
def main():
    ...
    images_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(here, "..", "source", "sokoban_embedded", "images")
    for skin, prefix in SKIN_PREFIX.items():
        for name in FULLSCREEN_IMAGES:
            ...
            print("%-8s %-12s %6d -> %6d bytes" % (skin, name, len(pixels) * 2, len(data)))
