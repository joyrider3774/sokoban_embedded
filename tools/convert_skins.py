...
"""Convert the skin images in assets to the flash data headers the game includes.
  ...
  assets/skins  -> source/sokoban_embedded/images   (16x16 tiles)
  assets/skins2 -> source/sokoban_embedded/images2  (8x8 tiles)

Full screen images (background, titlescreen) become run length encoded headers
(<name>_RLE565.h, see png2rle565.py), the other images become raw RGB565 little endian
headers (<name>_RGB565_LE.h, see png2rgb565.py).

Usage:
  python convert_skins.py            convert every set, overwriting the headers
  ...
  --set images2                      only handle this set (images or images2), can repeat
"""
import argparse
import filecmp
import os
...
SETS = {
    ...
    "images2": os.path.join(ROOT, "assets", "skins2"),
}
SOURCE_DIR = os.path.join(ROOT, "source", "sokoban_embedded")


def header_name(png):
    """File name of the header a skin image converts to, None if it is not converted."""
    name = png[:-4]
    if name in png2rle565.FULLSCREEN_IMAGES:
        return name + "_RLE565.h"
    return name.replace("-", "_") + "_RGB565_LE.h"
...
def convert_set(skins_dir, out_dir):
    """Converts every skin image of one set, returns {skin/header: source png} and skipped pngs."""
    made = {}
    skipped = []
    for skin in png2rgb565.SKIN_PREFIX:
        ...
        for png in sorted(os.listdir(os.path.join(skins_dir, skin))):
            if not png.lower().endswith(".png"):
                continue
            if header_name(png) is None:
                skipped.append("%s/%s" % (skin, png))
                continue
...
def verify_set(name, skins_dir, images_dir):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        made, skipped = convert_set(skins_dir, tmp)
    ...
    for png in skipped:
        print("  SKIPPED  %s (not a full screen or tile image)" % png)
    return not (different or missing or extra)
...
def main():
    ...
    parser.add_argument("--set", action="append", choices=sorted(SETS), help="only this set (can repeat)")
    args = parser.parse_args()

    ok = True
    for name in args.set or list(SETS):
        skins_dir = SETS[name]
        images_dir = os.path.join(SOURCE_DIR, name)
        if args.verify:
            ok &= verify_set(name, skins_dir, images_dir)
        else:
            made, skipped = convert_set(skins_dir, images_dir)
